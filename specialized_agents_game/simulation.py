...
def referential_game():
    num_agents_senders = 4
    num_agents_receivers = 4

    # Create linguistic community with separate lists for senders and receivers
    senders = [Sender() for _ in range(num_agents_senders)]
    receivers = [Receiver() for _ in range(num_agents_receivers)]

    # Split agents into two groups
    group_1_senders = senders[:2]
    group_2_senders = senders[2:]
    group_1_receivers = receivers[:2]
    group_2_receivers = receivers[2:]

    # Phase 1: Agents interact only within their groups
    print("Phase 1: Agents interacting within their groups")
    training(group_1_senders, group_1_receivers, 2)
    training(group_2_senders, group_2_receivers, 2)

    # Phase 2: Agents interact across groups
    print("Phase 2: Agents interacting across groups")
    training(senders, receivers, 4)
...
def training(senders, receivers, num_simulation_epochs):
    # num_simulation_epochs = 4
    ...
    for simulation_epoch in range(num_simulation_epochs):
        ...
        for _ in range(num_training_steps_per_epoch):
            sender_agent, receiver_agent = select_agents(senders, receivers)
...
def select_agents(senders, receivers):
    sender_idx = np.random.choice(len(senders))
    receiver_idx = np.random.choice(len(receivers))

    agent_pair = (senders[sender_idx], receivers[receiver_idx])
    return agent_pair
